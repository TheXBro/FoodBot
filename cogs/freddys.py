...
class freddys(commands.Cog):
    ...
    @commands.hybrid_command(name = cogname,
                             description = "Free double burger, frozen custard, or Oreo sandwich")   
    async def freddys(self, ctx: commands.Context, name: Optional[str]):
        await ctx.defer()

        await check_cooldowns(self, ctx, cogname, self.bot.freddys_cd)
        ...
        async with CloudflareScraper(connector=connector) as session:
            ...
            data = {
                    'user': {
                            ...
                            'anniversary': None,
                            'birthday': f"{str(random.randint(1950, 1999))}-{datetime.datetime.now().strftime('%m')}-{datetime.datetime.now().strftime('%d')}",
            ...
            try:
                response = await session.post('https://nomnom-prod-api.freddys.com/punchhv2/create', 
                    headers=headers, json=data, ssl=False, timeout=20)
            except:
                return await command_error(self, ctx, botmsg, self.bot.freddys_cd, cogname, connector, "RequestTimeout")

            result = await response.text()

        if random_email in result:
            embed = discord.Embed(title="F̷r̷e̷d̷d̷y̷s̷", url="https://www.freddys.com/account/login",
                description=f"Login app or site for a **free double steak burger or frozen custard**\nand a **free Oreo sandwich**.\n\n" #edit
            ...
            await command_error(self, ctx, botmsg, self.bot.freddys_cd, cogname, connector, "BadResponse")
        
async def setup(bot: commands.Bot):
    await bot.add_cog(freddys(bot))
